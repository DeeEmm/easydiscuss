...
class EasyDiscussModelClusters extends EasyDiscussAdminModel
{
	...
	public function __construct()
	{
		...
		$limit = $this->app->getUserStateFromRequest('com_easydiscuss.clusters.limit', 'limit', $this->app->getCfg('list_limit'), 'int');
	...
	/**
	 * Method to retrieve a pagination object
	 *
	 * @since	5.0.0
	 * @access	public
	 */
	...
	/**
	 * Method to retrieve user's cluster given by cluster type
	 *
	 * @since	6.0.0
	 * @access	public
	 */
	public function getUserCluster($userId, $clusterType = 'group')
	{
		$db = $this->db;

		$query = 'select a.`cluster_id` from `#__social_clusters_nodes` as a';
		$query .= '	inner join `#__social_clusters` as b on a.`cluster_id` = b.`id`';
		$query .= '		and b.`cluster_type` = ' . $db->Quote($clusterType) . ' and b.`state` = ' . $db->Quote(SOCIAL_STATE_PUBLISHED);
	...
	public function getPostsCluster($options)
	{
		...
		foreach($clusterId as $id) {
			$query = "select $id as `cluster_id`, a.`post_id`, a.`has_polls` as `polls_cnt`, a.`num_fav` as `totalFavourites`, a.`num_replies`, a.`num_attachments` as attachments_cnt,";
			...
			$query .= " and a.`cluster_id` = " . $db->Quote($id);
