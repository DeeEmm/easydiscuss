...
class EasyDiscussMaintenanceScriptUpdateThreadTableEngine extends EasyDiscussMaintenanceScript
{
	public static $title = 'Update thread db table engine to innodb' ;
	public static $description = 'Update thread table storage engine';
	...
	public function main()
	{
		...
		if ($defaultEngine != 'myisam' && $requireConvert) {
			...
			try {
				...
				$query = "ALTER TABLE `#__discuss_thread` engine=InnoDB";
				$db->setQuery($query);
				$db->query();
	...
	private function isRequireConvertion()
	{
		...
		try {
			$query = "SHOW TABLE STATUS WHERE `name` LIKE " . $db->Quote('%_discuss_thread');
			$db->setQuery($query);
