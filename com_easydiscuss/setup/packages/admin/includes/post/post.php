...
class EasyDiscussPost
{
	...
	public function getLastReplier($useCache = true)
	{
		...
		if (!isset($_cache[$key]) || !$useCache) {
			...
			if (isset($this->last_user_id)) {
				...
				if (!$this->last_user_id && $this->last_poster_name) {
			...
			if (!isset($this->last_user_id) || is_null($user)) {
	...
	public function replyNotify()
	{
		...
		$posterEmail = $reply->getEmail();

		// if the reply submitted by a guest user/anonymous then need to get their email differently
		if (!$this->my->id && isset($owner->user)) {
	...
	public function addTags()
	{
		...
		$existingTags = $model->getPostTags($this->post->id, [
			'checkPublished' => false
		]);


		if (!empty($existingTags) && $this->isBeingApproved()) {
		...
		// Delete any current tags
		$model->deletePostTag($this->post->id);
